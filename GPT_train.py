...
# Importing the GPT modules from huggingface/transformers
...
def answering(question, model, tokenizer):
...
class DataSetClass(Dataset):
    ...
    def __getitem__(self, index, device='cuda'):
        ...
        # label_mask = gpt_label["attention_mask"].squeeze()

        # return {
        #     "input_ids": input_ids.to(device, dtype=torch.bool),
        #     "attention_mask": input_mask.to(device, dtype=torch.bool),
        #     "labels": label_ids.to(device, dtype=torch.bool)
        # }
...
def GPT_trainer(
    ...
    tokenizer = GPT2Tokenizer.from_pretrained('Wenzhong2.0-GPT2-3.5B-chinese', padding_side='right')
    tokenizer.pad_token = tokenizer.eos_token
    model = GPT2LMHeadModel.from_pretrained('Wenzhong2.0-GPT2-3.5B-chinese', load_in_8bit=True, device_map='auto')
...
if __name__ == '__main__':
    ...
    # 定义模型的参数
    model_params = {
        "MODEL": "Wenzhong2.0-GPT2-3.5B-chinese",  # 初始模型路径
        "TRAIN_BATCH_SIZE": 4,  # 训练batch size
        "VALID_BATCH_SIZE": 4,  # 评估batch size
        "TRAIN_EPOCHS": 10,  # 训练epoch数
    ...
    # model.cuda()
    # print(answering(text,model,tokenizer))
